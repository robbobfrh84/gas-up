...
function rowObj_builder(keys, range) {
  let id_index
  const row = {}
  const indices = {}
  keys.forEach( (key, i) => {
...
function get_all_row_data(gsheet, sheet, sheetId, rowId) {
  ...
  if (range.error) return { data_error: "Can't find row range."} // Needs to be call data_error, because error is likely used before this func is called.

  const keys = get_sheet_keys(sheet)
  const rowObj = rowObj_builder(keys, range)
  const note = sheet.getRange(rowAll.getRow(), rowObj.id_index+1)
  return { rowAll, range, rowObj, note }
}
